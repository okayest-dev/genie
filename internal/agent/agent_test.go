...
func TestRunTurnLogsTurnStarted(t *testing.T) {
	...
	if err := RunTurn(context.Background(), c, "test-model", "sys", "hi", &out, nil, nil, nil, nil, "", nil); err != nil {
...
func TestRunTurnLogsTurnCompleted(t *testing.T) {
	...
	var out bytes.Buffer
	if err := RunTurn(context.Background(), c, "m", "sys", "prompt", &out, nil, nil, nil, nil, "", nil); err != nil {
		t.Fatalf("RunTurn: %v", err)
	}
	logged := buf.String()
...
func TestRunTurnToolCallsExecutedSerially(t *testing.T) {
	...
	if err := RunTurn(context.Background(), mock, "m", "sys", "hi", &out, &errOut, nil, reg, nil, "", nil); err != nil {
		t.Fatalf("RunTurn: %v", err)
	}
...
func TestRunTurnDisabledToolReturnsError(t *testing.T) {
	...
	var out bytes.Buffer
	if err := RunTurn(context.Background(), mock, "m", "sys", "hi", &out, nil, nil, reg, nil, "", nil); err != nil {
		t.Fatalf("RunTurn: %v", err)
	}
