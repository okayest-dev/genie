...
// ModelInfo implements the optional llm.ModelInfoProvider seam. The Anthropic
// wire's Zen-compatible model catalog exposes no context window, so it returns
// a zero ModelInfo (unknown) rather than an invented window; callers fall back
