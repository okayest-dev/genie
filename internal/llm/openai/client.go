...
func init() {
	llm.RegisterWire(llm.WireOpenAI, func(baseURL, apiKey string, _ map[string]any) llm.Client {
...
// ModelInfo implements the optional llm.ModelInfoProvider seam. The
// OpenAI-compatible wire's model catalog exposes no context window, so it
// returns a zero ModelInfo (unknown) rather than an invented window; callers
// fall back to an explicit config override.
